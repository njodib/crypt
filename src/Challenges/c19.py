...
ctxts = [AES_CTR(key, nonce).encrypt(b64decode(c)) for c in txts]
...
def keystream_from_input(ind, guess):
    ...
    for j,c_j in enumerate(ctxts):
        p_j = xor(c_j, ks)
        p_j += b'?'*(len(c_j)-len(ks))
        print(j, p_j.decode())

# Input as 'index guess'. If it is correct, the other lines will look normal. If incorrect, the other lines will be obviously corrupted.
# Just keep guessing until you know the answer.
if __name__ == '__main__':
    ind, guess = 0,''
    while True:
        print()
        keystream_from_input(ind,guess)
        print()
